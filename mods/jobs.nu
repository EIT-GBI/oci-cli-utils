...
const TIMEOUT_EXIT_VALUE = 'nu_job_timeout'
...
export def await [...tags --timeout (-t):duration --silent (-s)]: nothing -> list<any> {
  ...
  loop {
    ...
    let is_done = job list | where description in $tags | is-empty
    if $is_done { break }
  }
